...
#This is before I completely redefined the way that the cards are drawn
...
class Card(object):
	...
	def update_pos(self, coord):
		try:
			self.position = coord
			self.hitbox = pygame.Rect(self.position, (self.image.get_width()*0.2, self.image.get_height()))
		except TypeError:
...
class Deck(object):
	...
	def display_hand(self, hand = self.hands[0])
		num = len(hand)
		if num == 1:
			screen.blit(self.cards[hand[0]].image(), align(self.cards[hiq[0]].image(), "bottom", "center"))
		else:
			first = align(pygame.rect.Rect((0,0), (168+(168*0.2*num), 243)), "bottom", "center")
			c = 0
			x, y = first
			for i in hand:
				screen.blit(self.hands[i].image, (x+168*0.1*c, y))
	...
	def display_hand(self, hand_num = 1):
		...
		if num == 1:
			...
			if num %2 == 0:
				...
				lpivot = self.cards[hiq[num/2]].image
				lpivot_coord = offset(prototype.image, -0.1, "horizontal", pivot_coord)
				rpivot = self.cards[hiq[num/2+1]].image
				rpivot_coord =  offset(prototype.image, 0.1, "horizontal", pivot_coord)

				ldistance = (num/2-1)*-0.2-0.1
				rdistance = 0.9
			elif num%2 == 1:
				first_half = hiq[0: num/2]
				second_half = hiq[num/2+1:]
			...
			for i in range(len(first_half)):
				the_coordinates = offset(prototype.image, ldistance+i*0.2, "horizontal", pivot_coord)
				if self.cards[first_half[i]].identifier in self.final_hover:
					the_coordinates = offset(prototype.image, 0.4, "vertical", the_coordinates)
			...
			for i in range(len(second_half)):
				the_coordinates = offset(prototype.image, rdistance+i*0.2, "horizontal", pivot_coord)

				if self.cards[second_half[i]].identifier in self.final_hover:
					the_coordinates = offset(prototype.image, 0.4, "vertical", the_coordinates)
	...
	def play(self):
		...
		print
		print self.hands[0]
		if len(self.final_hover) > 0:
			for i in self.final_hover:
				print i
				self.hands[0].remove(i)
				self.played_cards.append(i)
				self.final_hover.remove(i)
		print self.hands[0]
