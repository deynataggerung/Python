...
#Fix self.hands...
#I need to add the ability to pass when it's too high as well as a way to start a new round when the current one is over. Possibly an instance of a player class contining "passed, current highest, etc"
...
class Player(object):
	def __init__(self, num, cards):
		self.identity = "player%d" % (num)
		self.winner = False
		self.position = "Noble"
		self.hand = cards.hands[num]
...
while playing:
	...
	for ev in ev_list:
		...
		if The_Rules.turn > 0:
			...
			The_Rules.aiturn()
			The_Screen.display()
			print The_Rules.pile.identifier
